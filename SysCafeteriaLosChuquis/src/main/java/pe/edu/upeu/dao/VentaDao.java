...
import pe.edu.upeu.modelo.DetalleVentaTO;
...
public class VentaDao extends AppCrud {
    ...
    public void descontarStockProducto(DetalleVentaTO vdTX) {
        leerA = new LeerArchivo(TABLA_DETALLEPRODUCTO);
        Object[][] dataP = buscarContenido(leerA, 0, vdTX.getIdProducto());
        DetalleProductoTO dto = new DetalleProductoTO();
        dto.setStock(Double.parseDouble(String.valueOf(dataP[0][7])) - vdTX.getCantidad());
        leerA = new LeerArchivo(TABLA_DETALLEPRODUCTO);
        editarRegistro(leerA, 0, vdTX.getIdProducto(), dto);
    }
    ...
    public void reporteVentas() {
        ...
        try {
            for (int i = 0; i < dataP.length; i++) {
                ...
                if ((fechaV.after(formatFecha.parse(fechaI)) || fechaSeparada[0].equals(fechaI)) &&
